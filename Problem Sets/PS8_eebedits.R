...
setwd("C:/Users/Emily/Documents/GitHub/Applied_Quant_Finance/Problem Sets")
rm(list=ls())
source('C:/Users/Emily/Documents/GitHub/Applied_Quant_Finance/Problem Sets/PS8_Helpers.R')
set.seed(1)
...
instant_sharpes <- holder[,2:17]/sd_returns
n_out <- rep(0,1, 16)
total <- 0
for(i in 1:16) {
  ...
  for(j in 1:length(temp)) {
    if((temp[j] > sharpe_returns[i] + 3* sr_sd_normal[i]) || (temp[j] < sharpe_returns[i] - 3*sr_sd_normal[i])) {
      n_out[i] <- n_out[i] +1
    }
  }
}
...
mdd_val_e <- find_mdd(build_nav(val_everywhere))
mdd_mom_e <- find_mdd(build_nav(mom_everywhere))

mom_e_normal <- (1+mean_mom_e^2 / 2*sd_mom_e^2) * 1/(527-1)
mom_e_general <- (1/(527-1)) * (1 + (sr_mom_e^2 / 4) * (kurt_mom_e - 1) -
                                    sr_mom_e * skew_mom_e)
val_e_normal <- (1+mean_val_e^2 / 2*sd_val_e^2) * 1/(527-1)
val_e_general <- (1/(527-1)) * (1 + (sr_val_e^2 / 4) * (kurt_val_e - 1) -
                                  sr_val_e * skew_val_e)
...
vm_sr <- vm_mean / vm_sd
nav_values_e <- apply(combined_val_mom, 2, build_nav)
drawdown_list <- apply(nav_values_e, 2, find_mdd)
...
#Question o
all_factors <- ff[679:1073,]
cbf <- read_csv("PS8_Bab.csv")
cbf[,2:4] <- cbf[,2:4]/100
colnames(cbf)[1] <- "Date"
all_factors <- merge(all_factors, cbf[679:1073,],by="Date")
all_factors$MOME <- mom_everywhere
all_factors$VALE <- val_everywhere
all_factors$RF <- NULL
all_factors$HML <- NULL
all_factors$UMD <- NULL
cov_all <- cov(na.omit(all_factors[,2:10]))

all_means <- apply(na.omit(all_factors[,2:10]), 2, mean)
all_sds <- apply(na.omit(all_factors[,2:10]), 2, sd)

one_v <- rep(1,1,9)
tan_weights <- (ginv(cov_all) %*% all_means) / (as.vector(t(one_v) %*% ginv(cov_all) %*% all_means))
mvp_weights <- (ginv(cov_all) %*% one_v) / as.vector((t(one_v) %*% ginv(cov_all) %*% one_v))

mvp_return <- all_means %*% mvp_weights
tan_return <- all_means %*% tan_weights

mvp_sd <- (t(mvp_weights) %*% cov_all %*% mvp_weights)^(1/2)
tan_sd <- (t(tan_weights) %*% cov_all %*% tan_weights)^(1/2)
cov_two_port <- t(tan_weights) %*% cov_all %*% mvp_weights
...
factors <- data.frame(ret = all_means, sd = all_sds)

this_plot <- ggplot(data = efficient_frontier, aes(x = sd, y = ret)) + 
  geom_point(color="firebrick") + 
  geom_point(aes(x=sd, y = ret), data = factors, color="blue") + 
...
sharpe_all <- tan_return/tan_sd

#Question p
odd_months <- seq(1, 395, 2)
even_months <- seq(2, 394, 2)
odd_all <- na.omit(all_factors[odd_months,])
even_all <- na.omit(all_factors[even_months,])


odd_cov <- cov(na.omit(odd_all[,2:10]))
even_cov <- cov(na.omit(even_all[,2:10]))

odd_vinv <- ginv(odd_cov)
even_vinv <- ginv(even_cov)

odd_means <- apply(odd_all[,2:10], 2, mean)
even_means <- apply(even_all[,2:10], 2, mean)

odd_tan_weights <- (odd_vinv %*% odd_means) / (as.vector(t(one_v) %*% odd_vinv %*% odd_means))
even_tan_weights <- (even_vinv %*% even_means) / (as.vector(t(one_v) %*% even_vinv %*% even_means))

odd_returns <- even_all[,2:10] * odd_tan_weights
even_returns <- odd_all[,2:10] * even_tan_weights
